                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="w-full max-w-md">
                            ...
                            />
                            {errors.name && <p className="mt-2 text-red-600 text-sm dark:text-red-400">{errors.name}</p>}
                        </div>
                        <div className="space-x-3 mt-7">
                            <button
                                type="submit"
                                disabled={processing}
                                className="inline-flex items-center bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 shadow-sm px-4 py-[11px] rounded-md font-medium text-sm text-white transition-colors cursor-pointer disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600"
                            >
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center">
                            <label className="block font-medium text-gray-900 text-lg dark:text-gray-100">Permissions</label>
                            <div className="flex items-center gap-3">
                                ...
                                />
                                <h4 className="font-semibold text-green-600 text-sm dark:text-green-400">All Permissions</h4>
                            </div>
                        </div>
                        ...
                        {Object.entries(permissions).map(([category, modules]) => (
                            <div key={category} className="shadow-sm border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
                                ...
                                {Object.entries(modules).map(([module, perms]) => (
                                    <div key={module} className="bg-white dark:bg-black">
                                        {/* Module Row */}
                                        <div className="flex items-center gap-3 bg-gray-100 dark:bg-gray-900/80 px-4 py-3 border-gray-100 dark:border-gray-800 border-b transition-colors">
                                            <input
                                                type="checkbox"
                                                ...
                                                className="bg-white dark:bg-gray-900 border-gray-300 dark:border-gray-700 rounded w-4 h-4 text-indigo-600 dark:text-indigo-500 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400"
                                            />
                                            <h4 className="font-semibold text-green-600 text-lg dark:text-green-400 capitalize">{module}</h4>
                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-4 p-4">
                                            {perms.map((permission) => (
                                                <div key={permission} className="flex items-center">
                                                    ...
                                                    <label
                                                        htmlFor={permission}
                                                        className="bg-blue-100/50 hover:bg-blue-200/50 dark:bg-gray-800 dark:hover:bg-gray-700 ml-3 px-3 py-1 rounded font-medium text-gray-700 text-sm dark:text-gray-300 transition-colors cursor-pointer"
                                                    >
                                                        {permission}
                                                    </label>
                                                </div>
